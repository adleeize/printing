        ...
        <div class="row border-top">
            <div class="span12 padding10 bg-grayLighter">
                <h1>
                    ...
                    <small class="on-right">Edit</small> Kasir
                </h1>
                <div><p></p></div>
                <form method="post" action="<?php echo current_url(); ?>">
                    ...
                    <div class="input-control text<?php $error = form_error('nama'); echo!empty($error) ? ' error-state' : ''; ?>" data-role="input-control">
                        <input type="text" placeholder="Nama" name="nama" value="<?php echo set_value('nama', $kasir->uacc_name); ?>">
                        <button class="btn-clear" tabindex="-1" type="button"></button>
                        <?php echo $error; ?>
                    </div>
                    <lable>Email</lable>
                    <div class="input-control text<?php $error = form_error('email'); echo!empty($error) ? ' error-state' : ''; ?>" data-role="input-control">
                        <input type="text" placeholder="Email" name="email" value="<?php echo set_value('email', $kasir->uacc_email); ?>">
                        <button class="btn-clear" tabindex="-1" type="button"></button>
                        <?php echo $error; ?>
                    </div>
                    ...
                    <div class="input-control text<?php $error = form_error('identitas'); echo!empty($error) ? ' error-state' : ''; ?>" data-role="input-control">
                        <input type="text" placeholder="No Pegawai" name="identitas" value="<?php echo set_value('identitas', $kasir->uacc_no_peg); ?>">
                        <button class="btn-clear" tabindex="-1" type="button"></button>
                        <?php echo $error; ?>
                    </div>
                    ...
                    <div class="input-control text<?php $error = form_error('no_telp'); echo!empty($error) ? ' error-state' : ''; ?>" data-role="input-control">
                        <input type="text" placeholder="Nomor Telepon" name="no_telp" value="<?php echo set_value('no_telp', $kasir->uacc_phone); ?>">
                        <button class="btn-clear" tabindex="-1" type="button"></button>
                        <?php echo $error; ?>
                    </div>
                    <lable>Kota</lable>
                    <div class="input-control text<?php $error = form_error('kota'); echo!empty($error) ? ' error-state' : ''; ?>" data-role="input-control">
                        <input type="text" placeholder="Kota" name="kota" value="<?php echo set_value('kota', $kasir->uacc_kota); ?>">
                        <button class="btn-clear" tabindex="-1" type="button"></button>
                        <?php echo $error; ?>
                    </div>
                    <input type="submit" value="Submit" name="edit_kasir" class="bg-cyan fg-white" />
                </form>
            </div>
        </div>
    </div>
